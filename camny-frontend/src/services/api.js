...
const api = axios.create({
  baseURL: "http://localhost:5000", // remove /api because most backend routes are root level
  headers: { "Content-Type": "application/json" },
});

// Interceptor to automatically attach token if it exists
...
/* ---------- Auth ---------- */
export const login = (payload) => api.post("/api/auth/login", payload); // only auth keeps /api
...
export const updateDriver = (driverId, payload) =>
  api.put(`/admin/drivers/${driverId}`, payload);
...
export const updateRoute = (routeId, payload) =>
  api.put(`/admin/routes/${routeId}`, payload);
...
export const updateVehicle = (vehicleId, payload) =>
  api.put(`/admin/vehicles/${vehicleId}`, payload);
...
export const updateTicket = (ticketId, payload) =>
  api.put(`/admin/tickets/${ticketId}`, payload);
export const deleteTicket = (ticketId) =>
  api.delete(`/admin/tickets/${ticketId}`);

export const getAllTicketTemplates = (companyId) =>
  api.get(`/admin/ticket-templates/${companyId}`);
export const createTicketTemplate = (payload) =>
  api.post("/admin/ticket-templates", payload);
export const updateTicketTemplate = (templateId, payload) =>
  api.put(`/admin/ticket-templates/${templateId}`, payload);
export const deleteTicketTemplate = (templateId) =>
  api.delete(`/admin/ticket-templates/${templateId}`);

/* ---------- Driver ---------- */
export const getDriverAssignments = (driverId) =>
  api.get(`/driver/assignments/${driverId}`);

/* ---------- Passenger ---------- */
export const getPassengerTickets = (passengerId) =>
  api.get(`/passenger/tickets/${passengerId}`);
export const getAllPassengerRoutes = () => api.get("/passenger/routes");
export const getPassengerPayments = (passengerId) =>
  api.get(`/passenger/payments/${passengerId}`);
export const createPassengerPayment = (payload) =>
  api.post("/passenger/payments", payload);
